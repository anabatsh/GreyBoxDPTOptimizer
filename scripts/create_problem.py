...
def create_problem_sets(problem_class, d, n, kwargs, n_test, n_val, n_train, save_path):
    problems = [problem_class(d=d, n=n, seed=i, **kwargs) for i in range(n_test+n_val+n_train)]
...
def main(problem, kwargs, d, n, n_train, n_val, n_test, save_dir):
    problem_name = problem + ''.join([f"__{k}_{v}" for k, v in kwargs.items()])
    save_path = os.path.join(save_dir, problem_name)
    os.makedirs(save_path, exist_ok=True)

    problem_class = getattr(pbm, problem)
    create_problem_sets(
        problem_class, d, n, kwargs,
