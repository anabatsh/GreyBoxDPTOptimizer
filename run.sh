...
PROBLEM_ARGS=(
    ...
    "--n 2"                              # binary vectors
    "--problem_kwargs ${PROBLEM_KWARGS}" # extra params # "{'seed': 0}"
)
RUNNER_ARGS=(
    "--budget 10"          # maximum of calls to the target function
    "--n_exp 2"            # number of reruns for each solver
    "--save_dir results" # where the results would be saved
)
